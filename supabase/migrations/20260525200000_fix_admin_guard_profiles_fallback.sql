-- =============================================================================
-- Fix admin guard to fall back to profiles.type when user_metadata.type is absent
-- Part of GIV-280: Error on https://giveprotocol.io/admin
--
-- Root cause: The frontend AuthContext resolves admin userType from either
--   1. user.user_metadata.type  (JWT claim), or
--   2. profiles.type fallback (DB lookup when metadata is missing)
-- The Supabase RPCs only checked (1), so an admin whose account has
-- profiles.type='admin' but no user_metadata.type='admin' in their JWT
-- could pass the frontend route guard but be rejected by every admin RPC,
-- producing "Failed to load dashboard statistics from Supabase."
--
-- Fix: add is_admin_user() helper that mirrors the frontend logic, then
-- rebuild the three admin dashboard RPCs to use it.
-- =============================================================================

-- ---------------------------------------------------------------------------
-- 1. is_admin_user() — shared admin check (JWT first, profiles fallback)
-- ---------------------------------------------------------------------------
...
AS $$
BEGIN
  -- Fast path: JWT metadata claim is present (normal case after proper signup)
  IF auth.jwt() -> 'user_metadata' ->> 'type' = 'admin' THEN
    RETURN TRUE;
  END IF;

  -- Fallback: check the profiles table directly.
  -- SECURITY DEFINER means we execute as the function owner (superuser),
  -- bypassing RLS, so this is safe and cannot recurse.
  RETURN EXISTS (
    SELECT 1
    FROM profiles
...
COMMENT ON FUNCTION public.is_admin_user() IS
  'Returns TRUE if the current caller is an admin, checked via JWT user_metadata '
  'first and profiles.type as a fallback. Mirrors the frontend AuthContext '
  'resolveUserType() logic. Part of GIV-280.';
...
-- ---------------------------------------------------------------------------
-- 2. get_admin_dashboard_stats() — updated guard
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- Total donors (profiles with type = 'donor')
  SELECT COUNT(*)
  INTO v_total_donors
  FROM profiles
  WHERE type = 'donor';

  -- Total charities (profiles with type = 'charity')
  SELECT COUNT(*)
  ...
  -- Total volunteers (profiles with type = 'volunteer')
  SELECT COUNT(*)
...
COMMENT ON FUNCTION public.get_admin_dashboard_stats() IS
  ...
  'Requires admin role (JWT metadata or profiles fallback). Part of GIV-280.';
...
-- ---------------------------------------------------------------------------
-- 3. get_admin_recent_activity() — updated guard
-- ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION public.get_admin_recent_activity(INT, INT) IS
  ...
  'verification events. Requires admin role (JWT metadata or profiles fallback). '
  'Part of GIV-280.';

REVOKE ALL ON FUNCTION public.get_admin_recent_activity(INT, INT) FROM PUBLIC;
...
-- ---------------------------------------------------------------------------
-- 4. get_admin_alerts() — updated guard
-- ---------------------------------------------------------------------------
