...
--   (profiles.id is auto-generated by the on_auth_user_created_profile
--   trigger, not the feed-b00x auth user id, so clean up via user_id.)
...
DECLARE
  ...
  -- Populated from the trigger-created profiles row (see below); profiles.id
  -- is auto-generated, so these have no fixed value.
  v_profile_id_1 UUID;
  v_profile_id_2 UUID;
BEGIN
  ...
  -- The auth.users insert above already fired the on_auth_user_created_profile
  -- trigger, which auto-created a profiles row (its own generated id, type
  -- from raw_user_meta_data). Update it with the display name and backdated
  -- created_at, and capture its id for the FK references below.
  UPDATE profiles
  SET name = 'Green Earth Alliance',
      created_at = NOW() - INTERVAL '6 days'
  WHERE user_id = v_user_id_1
  ...
  -- Same as charity 1: update the trigger-created profiles row rather than
  -- inserting a second row for the same user_id.
  UPDATE profiles
  SET name = 'Solar Aid Foundation',
      created_at = NOW() - INTERVAL '2 days'
  WHERE user_id = v_user_id_2
