export default {
  translation: {
    ...
    "privacy.security.auditLog":
      "GDPR 제32조에 따른 기술적 조치로서, 당사는 개인 데이터에 대한 관리자 접근을 기록하고 모니터링합니다. 여기에는 수행된 조치와 발생 시점이 포함되며, 무단 접근을 탐지하고 책임을 입증하며 개인 데이터 침해(제34조)가 발생할 가능성이 낮은 경우 사건을 재구성하기 위함입니다. 기부자의 개인 데이터는 이러한 관리자 접근 로그에 복제되지 않습니다. 관리자 접근 로그는 §7.1에 설명된 인증 및 보안 로그와 구별되며, 보존 기간은 당사 내부 처리 활동 기록에 의해 관리됩니다.",
  },
};
