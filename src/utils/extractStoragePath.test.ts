...
describe("extractStoragePath", () => {
  it("extracts path from a Supabase public URL", () => {
    const url =
      "https://abc.supabase.co/storage/v1/object/public/charity-attestations/charity-123/doc.pdf";
    expect(extractStoragePath(url, BUCKET)).toBe("charity-123/doc.pdf");
  });

  it("extracts path from a Supabase signed URL", () => {
    const url =
      "https://abc.supabase.co/storage/v1/object/sign/charity-attestations/charity-456/attestation.pdf?token=abc123";
  ...
  it("extracts path from a relative URL containing the bucket name", () => {
    const url =
      "/storage/v1/object/public/charity-attestations/profile-789/file.png";
    expect(extractStoragePath(url, BUCKET)).toBe("profile-789/file.png");
  });

  it("handles deeply nested paths", () => {
    const url =
      "https://abc.supabase.co/storage/v1/object/public/charity-attestations/org/sub/deep/file.pdf";
  ...
  it("handles URL with only bucket name and single file", () => {
    const url =
      "https://abc.supabase.co/storage/v1/object/public/charity-attestations/file.pdf";
