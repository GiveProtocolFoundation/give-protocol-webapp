...
describe("findErasureTargets", () => {
  ...
  async function findErasureTargets(
    supabase: ReturnType<typeof makeMockSupabase>,
  ): Promise<ErasureTarget[]> {
    const { data, error } = (await supabase
      .from("erasure_requests")
      .select(
        "id, user_id, requested_at, profiles!inner(scheduled_for_deletion_at)",
      )
      .eq("status", "pending")
    ...
    for (const row of data) {
      ...
      const email =
        (authUser as { user?: { email?: string } } | null)?.user?.email ?? "";
      targets.push({
        userId: row.user_id,
        email,
        requestId: row.id,
  ...
  it("returns targets with email from auth.users", async () => {
    ...
    (
      supabase.auth.admin.getUserById as ReturnType<typeof jest.fn>
    ).mockResolvedValue({
  ...
  it("throws when query returns an error", async () => {
    ...
    await expect(findErasureTargets(supabase)).rejects.toThrow(
      "Failed to query erasure targets",
    );
  });
});
...
describe("collectBlockchainRefs", () => {
  ...
  async function collectBlockchainRefs(
    ...
    const selfReportedResult = await (
      supabase.from("self_reported_hours") as unknown as MockChainBuilder
    )
      .select("id, blockchain_tx_hash, sbt_token_id")
      .eq("volunteer_id", userId)
      .not("blockchain_tx_hash", "is", null);

    const verifResult = await (
      supabase.from("volunteer_verifications") as unknown as MockChainBuilder
    )
    ...
    for (const row of (
      selfReportedResult as unknown as {
        data: Array<{
          id: string;
    ...
    if (refs.length > 0) {
      ...
      if (error)
        throw new Error(
          `sbt_audit_log insert failed: ${(error as { message: string }).message}`,
        );
    }
  ...
  it("returns empty refs when user has no blockchain records", async () => {
    ...
    (supabase._chainable.not as ReturnType<typeof jest.fn>).mockResolvedValue({
      data: [],
      error: null,
    });

    const { refs, stepsCompleted } = await collectBlockchainRefs(
      "user-1",
      supabase,
    );
    ...
    const insertCalls = (
      supabase._chainable.insert as ReturnType<typeof jest.fn>
    ).mock.calls;
    ...
    (supabase._chainable.not as ReturnType<typeof jest.fn>).mockImplementation(
      () => {
        ...
        if (callCount === 1) {
          ...
          return Promise.resolve({
            data: [
              { id: "row-1", blockchain_tx_hash: "0xabc", sbt_token_id: 42 },
            ],
            error: null,
          });
        }
...
describe("executeTransactionalErasure (Steps 2–9)", () => {
  ...
  async function executeTransactionalErasure(
    ...
    if (error)
      throw new Error(
        `Transactional erasure RPC failed: ${(error as { message: string }).message}`,
      );
  ...
  it("calls execute_gdpr_erasure RPC with correct parameters", async () => {
    const supabase = makeMockSupabase();
    const steps = await executeTransactionalErasure(
      "user-1",
      "alice@example.com",
      supabase,
    );
  ...
  it("throws when RPC returns an error", async () => {
    ...
    await expect(
      executeTransactionalErasure("user-3", "charlie@example.com", supabase),
...
describe("writeDeletionAuditLog (Step 11)", () => {
  ...
  async function writeDeletionAuditLog(
    ...
    const blockchainRefsJson =
      blockchainRefs.length > 0
    ...
    if (error) {
      console.error(
        `Step 11 deletion_audit_log write failed: ${(error as { message: string }).message}`,
      );
    }
  }
  ...
  it("does not throw if deletion_audit_log insert fails (non-fatal)", async () => {
    ...
    await expect(
      writeDeletionAuditLog(target, [], [], supabase),
...
describe("auth deletion failure compensating re-queue", () => {
  it("resets erasure_request to pending status when auth deletion fails", async () => {
    ...
    (supabase._chainable.update as ReturnType<typeof jest.fn>).mockReturnValue(
      updateChain,
    );
    ...
    expect(supabase._chainable.update).toHaveBeenCalledWith({
      ...
      error_message: expect.stringContaining(
        "Auth deletion failed (will retry)",
      ),
    });
  ...
  it("marks erasure_request as failed on unexpected error", async () => {
    ...
    expect(supabase._chainable.update).toHaveBeenCalledWith(
      expect.objectContaining({
        status: "failed",
        error_message: "Unexpected DB error",
      }),
    );
  });
});
  ...
  async function deleteCharityAttestationStorage(
    ...
    for (const wallet of wallets ?? []) {
      ...
      const path = extractStoragePath(
        wallet.custodian_attestation_doc_url,
        bucket,
      );
      ...
      data: [
        {
          custodian_attestation_doc_url:
            "https://abc.supabase.co/storage/v1/object/public/charity-attestations/cp-1/doc.pdf",
  ...
  it("includes charity_wallets in export package with correct shape", () => {
    ...
    const mapped = {
      ...
      proof_of_control_verified_at:
        walletRow.proof_of_control_verified_at ?? null,
