...
export const SelfReportedHoursStats: React.FC<SelfReportedHoursStatsProps> = ({
  ...
  const statCards = [
    ...
    },
    {
      ...
      count:
        stats.recordsByStatus[ValidationStatus.UNVALIDATED] +
        stats.recordsByStatus[ValidationStatus.EXPIRED],
      icon: HelpCircle,
      iconBg: "bg-gray-100",
  ...
  return (
    <div
      className={`grid gap-4 ${visibleCards.length === 4 ? "md:grid-cols-4" : "md:grid-cols-3"}`}
    >
            ...
            <div>
              ...
              <p className={`text-2xl font-semibold ${stat.valueColor}`}>
                ...
                <span className="text-sm font-normal text-gray-500 ml-1">
                  {t("volunteer.hoursUnit", "hrs")}
                </span>
              </p>
              <p className="text-xs text-gray-400">
                {stat.count}{" "}
                {stat.count === 1
                  ? t("volunteer.recordSingular", "record")
                  : t("volunteer.recordPlural", "records")}
              </p>
            </div>
          </Card>
        );
